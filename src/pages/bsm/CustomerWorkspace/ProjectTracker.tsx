  ...
  return (
    <div className="clay-card p-8">
      ...
      <div className="relative">
        <div className="absolute top-0 bottom-0 left-[23px] w-0.5 bg-slate-200/60" />
            ...
            return (
              <div key={step} className={`flex items-start gap-6 group ${isPending ? 'opacity-50' : ''}`}>
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 border-4 border-white shadow-sm transition-transform ${
                  ...
                  isCurrent ? 'bg-primary text-white scale-110 shadow-primary/20 shadow-md' :
                ...
                <div className={`flex-1 p-5 rounded-[1.5rem] border transition-all ${
                  isCurrent ? 'bg-white/90 border-primary/40 shadow-md' : 
                  isCompleted ? 'bg-white/80 border-emerald-300/40 shadow-sm' : 
                  'bg-white/50 border-slate-200/50 shadow-inner hover:border-slate-300'
                }`}>
                  <div className="flex items-center justify-between">
                    ...
                    {isCurrent && (
                      <span className="px-3 py-1 bg-white text-primary text-[10px] font-black uppercase tracking-widest rounded-full shadow-sm">
                        In Progress
                      </span>
                    )}
                  </div>

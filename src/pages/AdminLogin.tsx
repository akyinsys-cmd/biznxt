...
import { Mail, Lock, ArrowRight, ShieldAlert, Eye, EyeOff, ShieldCheck, KeyRound, Loader2, ArrowLeft } from 'lucide-react';
import { useNavigate, Link } from 'react-router-dom';
import { doc, getDoc } from 'firebase/firestore';
import { db } from '../lib/firebase';
import { emailSignIn, signInWithGoogle, resetPassword, logout, getRoleFromEmail } from '../lib/auth';
...
export default function AdminLogin() {
  ...
  useEffect(() => {
    ...
    const savedRemember = localStorage.getItem('admin_remember_me') !== 'false';
    if (savedRemember) {
      setRememberMe(true);
      if (savedEmail) {
        setEmail(savedEmail);
  ...
  // Autofocus the first field (Email) on mount or toggle mode
  useEffect(() => {
  ...
  const handleAdminAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isForgotPassword) {
        ...
        // Remember session credentials if requested
        if (rememberMe) {
        ...
        // Fallback to email-based role resolution in case of firestore latency or missing doc
        if (!userRole) {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      // Fallback to email-based role resolution
      if (!userRole) {
      ...
      showToastError('Google Auth failed');
          } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex-1 flex bg-[#0B1120] min-h-screen font-sans">
      <div className="w-full flex items-center justify-center p-4 sm:p-8 relative">
        ...
        {/* Back to Home Link */}
        <Link to="/" className="absolute top-8 left-8 text-slate-400 hover:text-white flex items-center gap-2 text-sm font-medium z-20">
          ...
          <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl shadow-2xl p-8 relative overflow-hidden">
            ...
            <form onSubmit={handleAdminAuth} className="space-y-5">
              ...
              <div className="flex items-center justify-between mt-2">
                <label className="flex items-center cursor-pointer group">
                  <input 
                    ...
                    className="w-4 h-4 rounded border-slate-700 text-blue-500 focus:ring-blue-500/50 bg-slate-800/50" 
                  />
              ...
              <div className="pt-2">
                ...
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (isForgotPassword ? 'Send Reset Link' : 'Secure Login')}
                  {!loading && !(isForgotPassword) && <ArrowRight className="ml-2 w-4 h-4" />}
                </button>
              </div>

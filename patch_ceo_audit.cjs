...
content = content.replace(/<div key=\{log\.id\} className="flex gap-4 p-4 border border-slate-100 rounded-2xl hover:border-slate-200 bg-slate-50">[\s\S]*?<\/div>\n\s*<\/div>/, enhancedAudit + '                    ');

fs.writeFileSync('src/pages/CEOCommandCenter.tsx', content);
console.log("Patched CEO Command Center Audit render");
